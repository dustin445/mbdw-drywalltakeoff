...
import { provisionJob, uploadFileToDrive } from "./googleApi.js";
...
const initJob = (name, jobNumber = "", type = "single", defaultMaterials = DEFAULT_MATERIALS_FALLBACK) => ({
  ...
  driveFolder: null, // { folderId, folderUrl, folderName } — set after Google Drive provisioning
});
...
export default function TakeoffApp() {
  ...
  const [showBudgetPw, setShowBudgetPw] = useState(false);
  const [passwordInput, setPasswordInput] = useState("");
  const [passwordError, setPasswordError] = useState(false);
  const [showMainPw, setShowMainPw] = useState(false);
  ...
  const [driveFailedJob, setDriveFailedJob] = useState(null); // job that failed Drive provisioning
  ...
  const createJob = async (skipNumberCheck = false) => {
    ...
    // Create a Google Drive sub-folder and log it to the spreadsheet.
    // This runs after navigation so the UI is never blocked.
    try {
      const driveFolder = await provisionJob(job);
      setJobs((prev) =>
        prev.map((j) => (j.id === job.id ? { ...j, driveFolder } : j))
      );
      setToast("Google Drive folder created");
    } catch (err) {
      console.error("Google Drive provisioning failed:", err);
      setDriveFailedJob(job);
    }
  };

  const retryDriveProvisioning = async () => {
    if (!driveFailedJob) return;
    setDriveRetrying(true);
    try {
      const driveFolder = await provisionJob(driveFailedJob);
      setJobs((prev) =>
        prev.map((j) => (j.id === driveFailedJob.id ? { ...j, driveFolder } : j))
      );
      setDriveFailedJob(null);
      setToast("Google Drive folder created");
    } catch (err) {
      console.error("Google Drive retry failed:", err);
    } finally {
  ...
  const handleSaveToGoogle = async () => {
    ...
    if (!currentJob.driveFolder?.folderId) {
      setToast("No Google Drive folder linked to this job");
      setTimeout(() => setToast(""), 3000);
      return;
    }
    const now = new Date();
    const timestamp = now.toISOString().slice(0, 16).replace("T", "_").replace(":", "-");
    const filename = `${currentJob.name.replace(/\s+/g, "_")}_${timestamp}.json`;
    const content = JSON.stringify(currentJob, null, 2);
    try {
      await uploadFileToDrive(currentJob.driveFolder.folderId, filename, content);
      setToast(`file saved to google folder: ${currentJob.driveFolder.folderName}`);
      setTimeout(() => setToast(""), 4000);
    } catch (err) {
      console.error("Save to Google failed:", err);
      setToast("Save to Google failed — see console");
      setTimeout(() => setToast(""), 3000);
    }
  };
  ...
  const isDisabledCell = (mat, len) => {
    ...
    // 12CD (ceiling board): no 9'
    if (mat.startsWith("12CD") && len === "9'") return true;
    // 14FL (flexible): 8' and 12' only
    if (mat.startsWith("14FL") && (len === "9'" || len === "10'" || len === "14'")) return true;
    // 12AR / 58AR (abuse resistant): 8', 10', 12' only
    if ((mat.startsWith("12AR") || mat.startsWith("58AR")) && (len === "9'" || len === "14'")) return true;
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 24px", width: "100%" }}>
          ...
          <div style={{ width: "100%", maxWidth: 320 }}>
            ...
            <div style={{ position:"relative" }}>
              <input
                autoFocus
                type={showMainPw ? "text" : "password"}
                style={{ ...styles.input, fontSize: 18, letterSpacing: 4, textAlign: "center", borderColor: passwordError ? "#ef4444" : "#1e293b", paddingRight: 40 }}
                placeholder="••••••••"
                value={passwordInput}
              ...
              />
              <button onClick={() => setShowMainPw(x => !x)} style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", color:"#64748b", fontSize:18, padding:4, touchAction:"manipulation", lineHeight:1 }}>{showMainPw ? "🙈" : "👁"}</button>
            </div>
            {passwordError && (
  ...
  // ── GLOBAL MODALS — checked before any screen return ─────────────────────
  ...
  const appFooter = (
    <footer style={{ textAlign: "center", padding: "12px 0 20px", marginTop: "auto", display: "flex", justifyContent: "center", gap: 20 }}>
      <a href="/change_log.html" target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: "#334155", textDecoration: "none", letterSpacing: "0.05em" }}>Changes</a>
      <a href="/app_info.html" target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: "#334155", textDecoration: "none", letterSpacing: "0.05em" }}>Info</a>
    </footer>
  );
      ...
      <div style={{ ...styles.shell, maxWidth: isLandscape ? "100%" : 520 }}>
        <div style={styles.header}>
          ...
          <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
            ...
            >⚙️</button>
            <button
              style={{ background: "none", border: "none", color: "#475569", fontSize: 11, fontWeight: 700, cursor: "pointer", padding: "0 4px", letterSpacing: "0.05em" }}
        ...
        {showBudgetUnlockModal && (
          <Modal title="🔒 Budget Access" onClose={() => { setShowBudgetUnlockModal(false); setBudgetPasswordInput(""); setBudgetPasswordError(false); }}>
            ...
            <div style={{ position:"relative" }}>
              <input
                autoFocus
                type={showBudgetPw ? "text" : "password"}
              ...
              />
              <button onClick={() => setShowBudgetPw(x => !x)} style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", color:"#64748b", fontSize:18, padding:4, touchAction:"manipulation", lineHeight:1 }}>{showBudgetPw ? "🙈" : "👁"}</button>
            </div>
      ...
      <div style={{ ...styles.shell, maxWidth: isLandscape ? "100%" : 520 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
            {(currentJob?.type === "single" || currentJob?.type === "budget") ? (
              <>
                ...
                <div style={{ display: "flex", gap: 4, justifyContent: "flex-end", width: "100%" }}>
                  <button
                    style={{ fontSize: 10, fontWeight: 700, padding: "5px 8px", borderRadius: 6, border: "none", cursor: "pointer", whiteSpace: "nowrap", background: "#4f46e5", color: "#fff", width: "100%" }}
        ...
        {driveFailedJob && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
            <div style={{ background: "#0d1526", borderRadius: 16, padding: 24, width: "100%", maxWidth: 420, boxSizing: "border-box", border: "1px solid #ef4444" }}>
              <div style={{ fontSize: 22, marginBottom: 8 }}>⚠️</div>
              <div style={{ fontWeight: 800, fontSize: 16, color: "#fca5a5", marginBottom: 10 }}>Google Drive Folder Not Created</div>
              <div style={{ fontSize: 14, color: "#94a3b8", marginBottom: 6 }}>
                Creating a Google Drive folder is a required step. The folder could not be created for:
              </div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#e2e8f0", background: "#1e293b", borderRadius: 8, padding: "8px 12px", marginBottom: 16 }}>
                {driveFailedJob.name} __ {driveFailedJob.id}
              </div>
              <div style={{ fontSize: 13, color: "#64748b", marginBottom: 20 }}>
                Check your internet connection and Google account access, then tap Retry.
              </div>
              <button
                onClick={retryDriveProvisioning}
                disabled={driveRetrying}
                style={{ width: "100%", padding: "14px", borderRadius: 10, border: "none", background: driveRetrying ? "#374151" : "#1d4ed8", color: "#fff", fontWeight: 800, fontSize: 15, cursor: driveRetrying ? "not-allowed" : "pointer", touchAction: "manipulation" }}
              >
                {driveRetrying ? "Retrying…" : "Retry"}
              </button>
            </div>
          </div>
        )}
                    ...
                    {LENGTHS.map((len) => {
                      ...
                      return (
                        <button
                          key={len}
                          disabled={disabled}
                          style={{ ...styles.cell, width: cellW, minWidth: cellW, height: cellH, fontSize: cellFontSize, background: disabled ? "#0a0f1e" : val > 0 ? "#1e3a5f" : "#1e293b", color: disabled ? "#1e293b" : val > 0 ? "#ffffff" : "#475569", cursor: disabled ? "not-allowed" : "pointer" }}
